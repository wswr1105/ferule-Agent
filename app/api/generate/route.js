  ...
  try {
    ...
    if (!provider.apiKey) {
      return Response.json(
        { error: `服务端未配置 ${provider.config.apiKeyEnv}。` },
        { status: 500 }
      );
    }
    ...
    const completion = await fetch(provider.config.url, {
      method: "POST",
      headers: {
...
${JSON_INSTRUCTIONS}`
    ...
    if (!completion.ok) {
      return Response.json(
        { error: getProviderErrorMessage(data, provider.config.label) },
...
function getProviderConfig() {
  ...
  const config = PROVIDERS[providerName] || PROVIDERS.deepseek;
  return {
    config,
...
function getProviderErrorMessage(data, providerLabel) {
  return (
    data.error?.message ||
    data.base_resp?.status_msg ||
    `${providerLabel} API 调用失败。`
  );
}
...
function parseJsonContent(content) {
  ...
  try {
    ...
    if (!match) {
      throw new Error("AI 返回内容不是有效 JSON，请稍后重试。");
    }
